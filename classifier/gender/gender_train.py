...
def main(mode='train', generated_data_path=None, checkpoint_path=None, config_path='configs/classifier/gender.json'):
    ...
    def forward_backward_log(data_loader, prefix="train"):
        ...
        labels = batch["gender"].to(training_args.device)
    ...
    if mode == 'train':
        ...
        for item in generated_data:
            gender, _, _ = attribute_parse(item["personal"], generated_data_path)
            ...
            labels = torch.tensor([gender]).to(training_args.device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train or run the gender classifier.")
    parser.add_argument("--mode", choices=("train", "inference"), default="train")
    parser.add_argument("--config", default="configs/classifier/gender.json")
