...
def main(mode='train', generated_data_path=None, checkpoint_path=None, config_path='configs/classifier/age.json'):
    ...
    logger.info("loding the dataset")
    ...
    def forward_backward_log(data_loader, prefix="train"):
        ...
        labels = batch["age"].to(training_args.device)
    ...
    if mode == 'train':
        ...
        for item in generated_data:
            _, age, _ = attribute_parse(item["personal"], generated_data_path)
            ...
            labels = torch.tensor([age]).to(training_args.device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train or run the age classifier.")
    parser.add_argument("--mode", choices=("train", "inference"), default="train")
    parser.add_argument("--config", default="configs/classifier/age.json")
