...
def get_args(model_path):
    ...
    if model_path.endswith(".json"):
        model_args, data_args, training_args, diffusion_args = parser.parse_json_file(
            json_file=os.path.abspath(model_path)
        )
    else:
        # 代码默认预设定的参数，但不一定能正常运行
        (
            model_args,
            data_args,
            training_args,
...
def get_summarization_args(model_path):
...
class ModelArguments:
    ...
    num_labels: Optional[int] = field(
        default=3,
        metadata={
            "help": "label number for classifier."
        },
    )    
    # h3
...
class Seq2SeqTrainingArguments(TrainingArguments):
    ...
    training_data_type: Optional[int] = field(
        default=None,
        metadata={
            "help": (
                "Comment or Seq2seq traing"
            )
        },
    )    
...
class DataTrainingArguments:
    ...
    max_persona_len: Optional[str] = field(
        default="test_file",
        metadata={
            "help": ("test dataset file")
        },
    )    
    max_article_len: Optional[int] = field(
        default=512,
        metadata={
            "help": ("max artcile info length")
        },
    )
    max_comment_len: Optional[int] = field(
        default=64,
        metadata={
            "help": ("max comment info length")
        },
    )
    limit_length: Optional[int] = field(
        default=600000,
        metadata={
            "help": ("training sample number")
        },
    )     
    ...
    ctr_opt_label_idx: int = field(
        default=2,
        metadata={
            "help": "# 0 (neg in sentiment), 1 (neutrality in sentiment), 2 (pos in sentiment)"
        },
    )
    decode_ctr_lr: float = field(
        default=2000.0,
        metadata={
            "help": "set to 0 for unconstrained generation, large value for controlled generation"
        },
    )
    ctr_model_name: str = field(
        default=None,
        metadata={
            "help": "classifier guidance"
        },
    )
    is_classifier_guidance: bool = field(
        default=False,
        metadata={
            "help": "classifier guidance"
        },
...
class DiffusionArguments:
    ...
    sampling_type: str = field(
        default="top_p",
        metadata={"help": "Logit projection strategy: `argmax`/`greedy` or `top_p`."},
    )
