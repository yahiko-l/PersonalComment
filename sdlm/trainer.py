...
from sdlm.pipelines.simplex_ddpm import SimplexDDPMPipeline, SimplexDDPMPipelineSeq2seq
...
class DiffusionTrainer(Trainer):
    ...
    def save_metrics(self, split, metrics, combined=True):
        # add the ensure_ascii=False

        split = self.annotated_split(split)
        """
        ...
        if self.args.do_train == False and self.args.do_eval == True:
            print(path)
            from sdlm.StandardOutput import standardprocessing
    ...
    def evaluation_loop(
        ...
        if self.training_args.training_data_type == "Seq2seq":
        ...
        elif self.training_args.training_data_type == "PersonaComment":
            pipeline = SimplexDDPMPipelineSeq2seq(
                model=model,
                scheduler=self.inference_noise_scheduler,
                simplex_value=self.diffusion_args.simplex_value,
                top_p=self.diffusion_args.top_p,
        ...
        for step, inputs in enumerate(dataloader):
            ...
            src_input = inputs['src_input']['input_ids']
            ...
            if src_input is not None:
                src_input = self._pad_across_processes(
                    src_input, pad_index=self.eos_token_id
                )
                src_input = self._nested_gather(src_input)
                src_host = (
                    src_input
                    if src_host is None
                    else nested_concat(
                        src_host, src_input, padding_index=self.eos_token_id
                    )
                )
            if loss is not None:
            ...
            if self.training_args.do_train == True:
                if step % 2 == 0:
                    break
        ...
        if src_host is not None:
            src_input = nested_numpify(src_host)
            all_src = (
                src_input
                if all_src is None
                else nested_concat(
                    all_src, src_input, padding_index=self.eos_token_id
                )
            )
        ...
        if all_src is not None:
            all_src = nested_truncate(all_src, num_samples)            
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        train_dataloader = DataLoader(self.train_dataset, 
                                    ...
                                    collate_fn=self.data_collator(self.tokenizer, self.data_args) if self.data_collator is not None else None,
                                    shuffle=True,
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        """
        valid_dataloader = DataLoader(self.eval_dataset, 
                                    batch_size=self.args.eval_batch_size,
                                    collate_fn=self.data_collator(self.tokenizer, self.data_args) if self.data_collator is not None else None,
                                    shuffle=True,
                                    num_workers=self.args.dataloader_num_workers)  
    ...
    def _rotate_checkpoints(self, use_mtime=False, output_dir=None) -> None:
        ...
        for checkpoint in checkpoints_to_be_deleted:
            ...
            if self.args.save_checkpoints_on_s3:
                ...
                command = f"aws s3 sync {checkpoint} s3://ai2-s2-research/rabeehk/{checkpoint}"
                os.system(command)
