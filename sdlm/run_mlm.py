...
from sdlm.data.dataset import PersonalCommentDataset_ML
from sdlm.data.dataset import collate_fn_with_ML_tokenizer as data_collator
...
def main(model_path):
    # parse args
    model_args, data_args, training_args, diffusion_args = get_args(model_path)
    ...
    log_level = 20          # 20: General logging; 30 Warning logging
    ...
    # initialize dataset & dataloader, truncation=True
    train_dataset = PersonalCommentDataset_ML(data_args.train_file, 
                                            tokenizer,
                                            data_args=data_args,)

    eval_dataset = PersonalCommentDataset_ML(data_args.validation_file,
                                            tokenizer,
                                            data_args=data_args,)

    test_dataset = PersonalCommentDataset_ML(data_args.test_file,
                                            tokenizer,
                                            data_args=data_args,)
    ...
    if training_args.do_eval:
        if training_args.load_states_in_eval_from_model_path:
            # model_args.model_name_or_path = "outputs/comment_robert/checkpoint-100000"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train or evaluate the base SDLM experiment.")
