...
class LongformerForDiffusionLM(LongformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `LongformerForMaskedLM` make sure `config.is_decoder=False` for "
        ...
        # NOTE: this is a roberta function
        ...
        if classifier_free_guidance or classifier_free_guidance_in_train:
            if self.config.classifier_free_simplex_inputs:
                if self.config.classifier_free_uncond_input == "empty_token":
                    empty_token_ids = self.get_longformer_empty_tokens(
    ...
    def resize_position_embeddings(
        ...
        old_position_embeddings_weight = (
            self.longformer.embeddings.position_embeddings.weight.clone()
        )

        padding_idx = self.config.pad_token_id
        self.longformer.embeddings.position_embeddings = nn.Embedding(
        ...
        with torch.no_grad():
            if num_position_embeds_diff > 0:
                self.longformer.embeddings.position_embeddings.weight[
                    :-num_position_embeds_diff
                ...
                self.longformer.embeddings.position_embeddings.weight = nn.Parameter(
                    old_position_embeddings_weight[:num_position_embeds_diff]
                )
        # move position_embeddings to correct device
        self.longformer.embeddings.position_embeddings.to(self.device)
        # Update other needed parameters.
        self.longformer.embeddings.position_ids = (
            torch.arange(self.config.max_position_embeddings)
            .expand((1, -1))
            .type_as(self.longformer.embeddings.position_ids)
        )
        self.longformer.embeddings.token_type_ids = torch.zeros(
            self.longformer.embeddings.position_ids.size(), dtype=torch.long
        ).type_as(self.longformer.embeddings.token_type_ids)
