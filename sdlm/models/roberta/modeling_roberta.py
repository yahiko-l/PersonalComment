...
from sdlm.models.roberta.paa_encoder import PretrainContentEncoder, PretrainPersonaEncoder
...
from .modeling_roberta_modified import RobertaLMHead as RobertaLMHead_modified
from .modeling_roberta_modified import RobertaModel as RobertaModel_modified
from .modeling_roberta_modified import RobertaPreTrainedModel as RobertaPreTrainedModel_modified

class RobertaForDiffusionSeq2seq(RobertaPreTrainedModel_modified):
    _keys_to_ignore_on_save = [r"lm_head.decoder.weight", r"lm_head.decoder.bias"]
    _keys_to_ignore_on_load_missing = [
        r"position_ids",
        r"lm_head.decoder.weight",
    ...
    def __init__(self, config):
        ...
        # config.max_position_embeddings = 514
        self.roberta = RobertaModel_modified(config, add_pooling_layer=False)
        self.lm_head = RobertaLMHead_modified(config)

        # # PAA pretrain-encoder 
        self.context_encoder = PretrainContentEncoder(self.name_or_path)
        self.persona_encoder = PretrainPersonaEncoder(self.name_or_path)
    ...
    def forward(
        ...
        context_encoder_hidden_states = self.context_encoder(**src_input)
        persona_encoder_hidden_states = self.persona_encoder(**persona_input)

        context_encoder_attention_mask = src_input['attention_mask']
        persona_encoder_attention_mask = persona_input['attention_mask']          
