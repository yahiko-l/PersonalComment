...
class GPT2ForDiffusionLM(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        """
        # simplex.shape == (batch_size, seq_len, vocab_size)
        inputs_probs = F.softmax(simplex, dim=-1)
        # inputs_probs.shape == (batch_size, seq_len, vocab_size)
    ...
    def resize_position_embeddings(
        ...
        old_position_embeddings_weight = (
            self.gpt2.embeddings.position_embeddings.weight.clone()
        )

        padding_idx = self.config.pad_token_id
        self.gpt2.embeddings.position_embeddings = nn.Embedding(
        ...
        with torch.no_grad():
            if num_position_embeds_diff > 0:
                self.gpt2.embeddings.position_embeddings.weight[
                    :-num_position_embeds_diff
                ...
                self.gpt2.embeddings.position_embeddings.weight = nn.Parameter(
                    old_position_embeddings_weight[:num_position_embeds_diff]
                )
        # move position_embeddings to correct device
        self.gpt2.embeddings.position_embeddings.to(self.device)
        # Update other needed parameters.
        self.gpt2.embeddings.position_ids = (
            torch.arange(self.config.max_position_embeddings)
            .expand((1, -1))
            .type_as(self.gpt2.embeddings.position_ids)
        )
        self.gpt2.embeddings.token_type_ids = torch.zeros(
            self.gpt2.embeddings.position_ids.size(), dtype=torch.long
        ).type_as(self.gpt2.embeddings.token_type_ids)
