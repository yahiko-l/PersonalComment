...
class SimplexDDPMPipelineSeq2seq(DiffusionPipeline):
    r"""
    This model inherits from [`DiffusionPipeline`]. Check the superclass documentation for the generic methods the
    library implements for all the pipelines (such as downloading or saving, running on a particular device, etc.)
    Parameters:
    ...
    def __init__(
        ...
        if self.data_args.is_classifier_guidance:
            self.data_args.ctr_model = AutoModelForSequenceClassification.from_pretrained(self.data_args.ctr_model_name).to(self.device)

    def apply_controlling_drift(self, args, perturbed_inputs_diralpha):
    ...
    def __call__(
        ...
        for t in self.progress_bar(self.scheduler.timesteps):
            ...
            model_output = self.model(
                ...
                persona_input=batch["persona_input"],
                timesteps=t_scaled,
                previous_pred=previous_pred
                if self.model.config.self_condition
                else None,
            ...
            model_output_logits = model_output.logits
            if self.data_args.is_classifier_guidance:
                model_output_logits = self.apply_controlling_drift(self.data_args, model_output_logits)
